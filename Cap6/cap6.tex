...
\subsection{Ferramentas aceitas (exemplos)}

São aceitas ferramentas que permitam gerar e editar PDF \textbf{localmente} (sem \textit{upload}), incluindo, por exemplo:

\begin{itemize}
    \item \textbf{Freeware (\ac{GUI})}: PDF24 Creator (Windows).
    \item \textbf{\ac{FOSS} (\ac{GUI} local / auto-hospedável)}: Stirling PDF (uso local).
    \item \textbf{\ac{FOSS} (\ac{CLI} / multiplataforma)}: \texttt{qpdf}, \texttt{pdftk} (ou \texttt{pdftk-java}).
    \item \textbf{Assinatura digital (freeware/\ac{FOSS})}: JSignPdf (Java).
\end{itemize}

\begin{quote}
Recomenda-se priorizar ferramentas locais/offline, sobretudo quando o documento contiver dados pessoais ou informações sensíveis.
\end{quote}
